...
Workers must run `quota-axi` before model selection and pass that intake snapshot plus explicit `harness:provider:model` candidates to `bin/fm-quota-choose.sh`; harness alone is never a provider proxy.
Firstmate can arm the quota procevent source with `bin/fm-procevent-quota.sh` to wake on exhaustion.
