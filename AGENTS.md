...
Workers must run `quota-axi` before any model selection and pass that intake snapshot to `bin/fm-quota-choose.sh`; firstmate can arm the quota procevent source with `bin/fm-procevent-quota.sh` to wake on exhaustion.
