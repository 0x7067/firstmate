...
SH
  ...
  commands=("prime-agent --flag" "command prime-agent --flag" "command -- prime-agent --flag" "exec prime-agent --flag" "env prime-agent --flag" "node $prime_package/dist/bundle/cli.js" "node --trace-warnings $prime_package/dist/bundle/cli.js" "node './Linked Prime/dist/bundle/cli.js'" "env -C $prime_package node dist/bundle/cli.js" "sh -c prime-agent" "bash -lc prime-agent" "bash -o posix -c prime-agent" "time -p prime-agent" "$time_bin -p prime-agent" "'$FAKEBIN_DIR/prime-agent' --flag" "p --flag" "prime-proxy --flag" "nohup prime-agent --flag" "nice -n 10 prime-agent --flag" "cd $prime_package; node dist/bundle/cli.js" "PATH=$FAKEBIN_DIR:\$PATH p --flag" "claude --flag;prime-agent" $'custom-agent --flag\nprime-agent --flag' 'claude --flag $(prime-agent)' 'echo "$(prime-agent)"' 'FOO="$(prime-agent)" custom-agent --flag' 'echo `prime-agent`' 'eval prime-agent' 'runner=prime-agent; $runner --flag' '$runner --flag' 'cat <(prime-agent)' 'cat >(prime-agent)' 'cat < <(prime-agent)' 'cat >$(prime-agent)' 'node "$PWD/Linked Prime/dist/bundle/cli.js"' "2>$CASE_DIR/prime.err prime-agent --flag" "if true; then prime-agent; fi" "envx prime-agent --flag" "/usr/bin/arch prime-agent --flag")
