...
The crewmate-only Prime Agent 0.8.0 adapter was verified separately on 2026-08-23 against tmux 3.6a on macOS arm64.
...
Prime did not emit Pi's `agent_settled` event, so the adapter uses `agent_end` as its idle edge and remains crewmate/scout-only.
...
A second live run wrote through `bin/fm-busy-event.sh` and read the stored contract through `fm_busy_classify`.
